...
import torch
import torch.utils.data
from torchneuromorphic.utils import load_ATIS_bin, load_jaer, load_mat
...
def create_events_hdf5(directory, hdf5_filename):
    ...
    print(f"FILE: {hdf5_filename}")
    print(f"FILE: {pathlib.Path(hdf5_filename)}")
    print(f"FILE: {pathlib.Path(hdf5_filename).absolute()}")
    print(f"CURRENT: {pathlib.Path.cwd()}")
    with h5py.File(pathlib.Path(hdf5_filename).absolute(), 'w') as f:
        f.clear()
        #key = 0
        num = 0
        metas = []
